...
namespace Timers
{
    ...
    [DisallowMultipleComponent]//防止重複添加管理器
    [DefaultExecutionOrder(-9999999)] // 此腳本將在其他所有腳本之前執行
    public class TimersMgr : MonoSingleton<TimersMgr>
    {
        //// Ensure we only have a single instance of the TimersManager loaded (singleton pattern).
        //private static TimersMgr m_instance = null;
        ...
        /// </summary>
        /// <param name="id">Timer id</param>
        public static float Interval(int id) { Timer timer = GetTimer(id); return timer == null ? 0f : timer.Interval; }

        /// <summary>
        /// Get total loops count (INFINITE (which is uint.MaxValue) if is constantly looping) 
        /// </summary>
        /// <param name="id">Timer id</param>
        public static uint LoopsCount(int id) { Timer timer = GetTimer(id); return timer == null ? 0 : timer.LoopsCount; }

        /// <summary>
        /// Get how many loops were completed
        /// </summary>
        /// <param name="id">Timer id</param>
        public static uint CurrentLoopsCount(int id) { Timer timer = GetTimer(id); return timer == null ? 0 : timer.CurrentLoopsCount; }

        /// <summary>
        /// Get how many loops remained to completion
        /// </summary>
        /// <param name="id">Timer id</param>
        public static uint RemainingLoopsCount(int id) { Timer timer = GetTimer(id); return timer == null ? 0 : timer.RemainingLoopsCount; }

        /// <summary>
        /// Get total remaining time
        /// </summary>
        /// <param name="id">Timer id</param>
        public static float RemainingTime(int id) { Timer timer = GetTimer(id); return timer == null ? -1f : timer.RemainingTime; }

        /// <summary>
        /// Get total elapsed time
        /// </summary>
        /// <param name="id">Timer id</param>
        public static float ElapsedTime(int id) { Timer timer = GetTimer(id); return timer == null ? -1f : timer.ElapsedTime; }

        /// <summary>
        /// Get elapsed time in current loop
        /// </summary>
        /// <param name="id">Timer id</param>
        public static float CurrentCycleElapsedTime(int id) { Timer timer = GetTimer(id); return timer == null ? -1f : timer.CurrentCycleElapsedTime; }

        /// <summary>
        /// Get remaining time in current loop
        /// </summary>
        /// <param name="id">Timer id</param>
        public static float CurrentCycleRemainingTime(int id) { Timer timer = GetTimer(id); return timer == null ? -1f : timer.CurrentCycleRemainingTime; }

        /// <summary>
        /// Verifies whether the timer exits
        /// </summary>
        /// <param name="id">Timer id</param>
        public static bool IsTimerActive(int id) { Timer timer = GetTimer(id); return timer != null; }

        /// <summary>
        /// Checks if the timer is paused
        /// </summary>
        /// <param name="id">Timer id</param>
        public static bool IsTimerPaused(int id) { Timer timer = GetTimer(id); return timer == null ? false : timer.IsPaused; }
        ...
        public static void SetPaused(int id, bool bPause) { Timer timer = GetTimer(id); if (timer != null) timer.SetPaused(bPause); }

        /// <summary>
        /// Get total duration, (INFINITE if it's constantly looping)
        /// </summary>
        /// <param name="id">Timer id</param>
        public static float Duration(int id) { Timer timer = GetTimer(id); return timer == null ? 0f : timer.Duration; }
    }
}